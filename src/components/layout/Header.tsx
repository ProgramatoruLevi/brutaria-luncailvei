...
import { Menu, X, Phone, MessageCircle, ArrowUpRight, Mail, Clock, MapPin, Home, Package, Info, Send } from 'lucide-react'
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-5 py-6">
                ...
                <motion.div
                  ...
                  className="flex items-center justify-between gap-3 mb-5 px-4 py-2.5 rounded-full bg-green-50 ring-1 ring-green-200"
                >
                  ...
                  <span className="text-[11px] font-medium text-green-700/80 whitespace-nowrap">
                    06:00 – 20:00
                  </span>
                </motion.div>
                ...
                <nav className="flex flex-col gap-2">
                  {mainNavigation.map((item, i) => {
                    const isActive = location.pathname === item.href
                    const Icon = navIcons[item.href] ?? ArrowUpRight
                    return (
                      <motion.div
                        key={item.href}
                      ...
                      >
                        <Link
                          to={item.href}
                          className={cn(
                            'group relative flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-200 overflow-hidden',
                            isActive
                        ...
                        >
                          <span
                            className={cn(
                              'inline-flex items-center justify-center w-11 h-11 rounded-xl shrink-0 transition-colors',
                              isActive
                                ? 'bg-white/15 text-white'
                                : 'bg-surface text-primary ring-1 ring-border/60 group-hover:ring-accent/40'
                            )}
                          >
                            <Icon className="w-5 h-5" />
                          </span>
                          <span className="font-serif text-lg font-semibold flex-1">
                            {item.label}
                          </span>
                          <span
                            className={cn(
